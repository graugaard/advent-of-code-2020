...
pub fn print_solution() {
		...
		let potential_max = puzzle.split_ascii_whitespace()
...
fn get_my_seat(str: &str) -> u32 {
		let set: HashSet<u32> = str.split_ascii_whitespace()
			.map(|s| determine_seat_id(s))
			.collect();
...
pub fn determine_seat_id(boarding_pass: &str) -> u32 {
		8 * determine_row(&boarding_pass[0..7])
			+ determine_column(&boarding_pass[7..])
